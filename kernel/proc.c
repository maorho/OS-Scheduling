...
found:
  ...
  for (pp = proc; pp < &proc[NPROC]; pp++) {
    if (pp->state == RUNNING || pp->state == RUNNABLE)
      min = pp->accumulator < min ? pp->accumulator: min;
  }
  p->accumulator = min;
  ...
  for(;;){
    ...
    for(pp = proc; pp < &proc[NPROC]; pp++){
      if(pp->parent == p){
        ...
        if(pp->state == ZOMBIE) {
          // Found one.
          if (exit_msg != 0 && copyout(p->pagetable, exit_msg, pp->exit_msg, sizeof(pp->exit_msg)) < 0) {
            release(&pp->lock);
            release(&wait_lock);
            return -1;
          }
...
ps_scheduler(void) 
{
  ...
  for(p = proc; p < &proc[NPROC]; p++) {
      acquire(&p->lock);
      if(p->state == RUNNABLE && (curr_min == -1 || p->accumulator < curr_min)) {
        if (found == 1) {
          release(&pp->lock);
        }
        pp = p;
        curr_min = p->accumulator;
        found = 1;
...
long long get_vruntime(struct proc *p)
{
  if (p->cfs_priority == 0) {
    return (75 * p->rtime) / (p->rtime + p->stime + p->retime);
  } else if (p->cfs_priority == 1) {
    return (100 * p->rtime) / (p->rtime + p->stime + p->retime);
  } else {
    return (125 * p->rtime) / (p->rtime + p->stime + p->retime);
  }
}
...
void cfs_scheduler(void)
{
  ...
  for(p = proc; p < &proc[NPROC]; p++) {
      ...
      if(p->state == RUNNABLE && (curr_min == -1 || get_vruntime(p) < curr_min)) {
        if (found == 1) {
          release(&pp->lock);
        }
        pp = p;
        curr_min = get_vruntime(p);
        found = 1;
        // Switch to chosen process.  It is the process's job
        // to release its lock and then reacquire it
        // before jumping back to us.
...
void
get_cfs_stats(int pid, uint64 cfs_priority, uint64 rtime, uint64 stime, uint64 retime) {
  struct proc *p;
  for (p = proc; p < &proc[NPROC]; p++) {
    if (pid == p->pid) {
